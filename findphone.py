#!/usr/bin/env python3
"""
Find My Phone — the host companion for the find-my-phone watchapp. The watchapp's UP button rings this
computer, DOWN stops it. A watchapp is used (rather than a notification) because the firmware only
offers a notification's action menu while it's on screen, whereas a watchapp's buttons work whenever
you open it.
...
See README.md — the short version is: `./package.sh` then `stoandl ext install findphone.tar.gz`.
Optional: set `extension.findphone.sound = /path/to/alarm.oga` in stoandl.conf.
"""
import os
import shlex
import signal
